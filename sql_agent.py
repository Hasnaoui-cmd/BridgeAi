...
db = SQLDatabase.from_uri(
    db_url,
    include_tables=["hs_nomenclature"],
    engine_args={"pool_pre_ping": True, "pool_recycle": 1800, "pool_timeout": 30}
)
...
1. SCHEMA DISCOVERY: Even if you think you know the table names, ALWAYS start by listing tables and then checking the schema for the specific columns. (Use sql_db_list_tables and sql_db_schema). NOTE: You ONLY have access to the 'hs_nomenclature' table. Do not attempt to query any other tables.
2. SQL STRATEGY: 
...
3. QUALITY CONTROL: Before giving the final answer, verify that every numeric value came directly from a database row.
