...
def get_live_weather(city: str) -> dict:
    ...
    if not WEATHERAPI_KEY:
        print(f"⚠️ [Weather] No WEATHERAPI_KEY configured. Defaulting for '{city}'.")
        return default
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"🌤️ [Weather] {city}: {condition_text} | {result['temp_c']}°C | {result['wind_kph']} kph → severity={severity}")
            return result
        else:
...
def get_mock_port_congestion(port_name: str) -> int:
    """
    ...
    print(f"🚢 [Congestion] {port_name}: congestion_score={score}")
    return score
